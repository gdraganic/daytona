...
import { PayloadForJobType, ResourceTypeForJobType } from '../dto/job-type-map'
...
export class JobService {
  ...
  constructor(
    ...
    @Inject(forwardRef(() => SandboxJobService))
    private readonly sandboxJobService: SandboxJobService,
  ) {}
  ...
  async updateJobStatus(jobId: string, status: JobStatus, errorMessage?: string): Promise<Job> {
    ...
    // Handle job completion for v3 runners - update sandbox/snapshot/backup state
    if (status === JobStatus.COMPLETED || status === JobStatus.FAILED) {
