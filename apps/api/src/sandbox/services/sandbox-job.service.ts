...
/**
 * Service for handling sandbox state updates based on job completion (v3 runners only).
 * This service listens to job status changes and updates sandbox state accordingly.
  ...
  async handleJobCompletion(job: Job): Promise<void> {
    ...
    switch (job.type) {
      ...
      case JobType.DESTROY_SANDBOX:
        ...
        break
      default:
        // Ignore other job types (snapshots, backups, etc.)
        break
    }
  }
